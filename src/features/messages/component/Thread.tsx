...
export const Thread = ({ messageId, onClose }: ThreadProps) => {
  ...
  const getMessages = useGetMessages({ channelId, parentMessageId: messageId });
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex flex-col-reverse pb-4 overflow-y-auto messages-scrollbar">
          ...
          <div key={dateKey}>
            ...
            {messages.map((message, index) => {
              ...
              const isCompact =
                ...
                dayjs(message.createdAt).diff(dayjs(prevMessage.createdAt), "minute") < TIME_THRESHOLD;
              return (
                <Message
                  key={message.id}
                  id={message.id}
          ...
          ref={(el) => {
            if (el) {
              const observer = new IntersectionObserver(
                ([entry]) => {
                  if (entry.isIntersecting && getMessages.status === "CanLoadMore") {
        ...
        <Message
          id={getMessage.data.id}
          memberId={getMessage.data.memberId}
          authorImage={getMessage.data.user.image ?? undefined}
          authorName={getMessage.data.user.name ?? undefined}
          ...
          createdAt={new Date(getMessage.data.createdAt).getTime()}
          isEditing={editingId === getMessage.data.id}
          setEditingId={setEditingId}
          hideThreadButton
          isAuthor={currentMember.data?.id === getMessage.data.memberId}
        />
      </div>
      <div className="px-4">
        <Editor
